...
async def pm_is_subscribed(bot, query):
  try:
    ...
    btn = [[
      InlineKeyboardButton("🤖 Join This Channel 🤖",
                           url=invite_link.invite_link)
    ]]
    if query.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
      await query.reply_text(
        text="**ᴘʟᴇᴀsᴇ ᴊᴏɪɴ ᴍʏ ᴄʜᴀɴɴᴇʟ ᴏʀ ᴜ ᴄᴀɴɴᴏᴛ ᴜsᴇ ᴍᴇ**",
...
async def give_filter(client, message):
  delet = await message.reply_text("**Searching...** __Please Wait__")
  result = await pm_is_subscribed(client, message)
  if result == False:
    await delet.delete()
    return
...
async def next_page(bot, query):
  ...
  if not search:
    await query.answer(
      "Yᴏᴜ ᴀʀᴇ ᴜꜱɪɴɢ ᴏɴᴇ ᴏғ ᴍʏ ᴏʟᴅ ᴍᴇꜱꜱᴀɢᴇꜱ, ᴘʟᴇᴀꜱᴇ ꜱᴇɴᴅ ᴛʜᴇ ʀᴇǫᴜᴇꜱᴛ ᴀɢᴀɪɴ.",
      show_alert=True)
    return

  files, n_offset, total = await get_search_results(search,
                                                    offset=offset,
                                                    filter=True)
  try:
  ...
  if settings['button']:
    ...
    btn = [[
      InlineKeyboardButton(text=f"{file.file_name}",
  ...
  btn.insert(
    0, [InlineKeyboardButton('🔋 Hᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ 🔋', url=f'{HOW_TO_LINK}')])
  btn.insert(1, [
    InlineKeyboardButton('DEV 1', url='https://t.me/rahul_thakor'),
    InlineKeyboardButton('DEV 2', url='https://t.me/Sharathitsisme')
  ])
  ...
  if k == False:
    ...
    if files:
      ...
      k = await query.message.edit(
        'ᴛʜɪs ᴍᴏᴠɪᴇs ɪs ɴᴏᴛ ᴀᴠᴀɪʟᴀʙʟᴇ ɪɴ ᴍʏ ᴅᴀᴛᴀʙᴀsᴇ ᴏʀ ᴛʀʏ ᴀɢᴀɪɴ ᴡɪᴛʜ ᴄᴏʀʀᴇᴄᴛ sᴘᴇʟʟɪɴɢ'
      )
...
async def cb_handler(client: Client, query: CallbackQuery):
  if query.data == "close_data":
      ...
      if grpid is not None:
        ...
        try:
          ...
          await query.message.edit_text(
            "Mᴀᴋᴇ ꜱᴜʀᴇ I'ᴍ ᴘʀᴇꜱᴇɴᴛ ɪɴ ʏᴏᴜʀ ɢʀᴏᴜᴘ!!", quote=True)
          return await query.answer()
      else:
        await query.message.edit_text(
    ...
    if (st.status == enums.ChatMemberStatus.OWNER) or (str(userid) in ADMINS):
      ...
      await query.answer(
        "Yᴏᴜ ɴᴇᴇᴅ ᴛᴏ ʙᴇ Gʀᴏᴜᴘ Oᴡɴᴇʀ ᴏʀ ᴀɴ Aᴜᴛʜ Uꜱᴇʀ ᴛᴏ ᴅᴏ ᴛʜᴀᴛ!",
        show_alert=True)
    ...
    if chat_type == enums.ChatType.PRIVATE:
      ...
      grp_id = query.message.chat.id
      st = await client.get_chat_member(grp_id, userid)
      if (st.status == enums.ChatMemberStatus.OWNER) or (str(userid)
                                                         in ADMINS):
        await query.message.delete()
        try:
          await query.message.reply_to_message.delete()
    ...
    if mkact:
      ...
      await query.message.edit_text('sᴏᴍᴇ ᴇʀʀᴏʀ ᴏᴄᴄᴜʀᴇᴅ!!!',
                                    parse_mode=enums.ParseMode.MARKDOWN)
    return await query.answer()
    ...
    if mkinact:
      ...
      await query.message.edit_text(f"Some error occurred!!",
                                    parse_mode=enums.ParseMode.MARKDOWN)
    return await query.answer()
    ...
    if delcon:
      await query.message.edit_text("sᴜᴄᴄᴇssғᴜʟʟʏ ᴅᴇʟᴇᴛᴇᴅ ᴄᴏɴɴᴇᴄᴛɪᴏɴs")
    else:
      await query.message.edit_text(f"sᴏᴍᴇ ᴇʀʀᴏʀ ᴏᴄᴄᴜʀs!!",
                                    parse_mode=enums.ParseMode.MARKDOWN)
    return await query.answer()
    ...
    for groupid in groupids:
      try:
        ...
        buttons.append([
          InlineKeyboardButton(text=f"{title}{act}",
                               callback_data=f"groupcb:{groupid}:{act}")
        ])
      except:
        pass
    if buttons:
      await query.message.edit_text("ʏᴏᴜʀ ɢʀᴏᴜᴘ ᴅᴇᴛᴀɪʟs ;\n\n",
  ...
  if query.data.startswith("file"):
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, query):
      await query.answer("I Lɪᴋᴇ Yᴏᴜʀ Sᴍᴀʀᴛɴᴇꜱꜱ, Bᴜᴛ Dᴏɴ'ᴛ Bᴇ Oᴠᴇʀꜱᴍᴀʀᴛ 😒",
                         show_alert=True)
      return
    ident, file_id = query.data.split("#")
    files_ = await get_file_details(file_id)
    ...
    buttons = [[
      InlineKeyboardButton('🤖 ᴜᴘᴅᴀᴛᴇs 🤖', url='https://t.me/Crazebots'),
      InlineKeyboardButton('💥 ᴍᴏᴠɪᴇs ♥️', url=f"{GROUP_LINK}")
    ],
               [
                 InlineKeyboardButton('💞 sᴜᴘᴘᴏʀᴛ 💞',
                                      url='https://t.me/Crazebots'),
                 InlineKeyboardButton(
                   '👩‍💻 ʀᴇᴘᴏ ✨',
                   url='https://GitHub.Com/Crazebotz/AwesomeFilterPro'),
               ],
               [
                 InlineKeyboardButton('👋 ᴇxᴛʀᴀ ʙᴜᴛᴛᴏɴs 👋',
                                      callback_data='about'),
               ],
    ...
    await query.message.edit_text(text="▣ ▣ ▣")
    await query.message.edit_text(text=script.ABOUT_TXT.format(temp.U_NAME),
                                  reply_markup=reply_markup,
                                  parse_mode=enums.ParseMode.HTML)
...
async def auto_filter(client, msg, spoll=False):
  ...
  if settings["button"]:
    btn = [[
      InlineKeyboardButton(
        text=f"[{get_size(file.file_size)}] {file.file_name}",
        url=await get_shortlink(
          f"https://telegram.me/{temp.U_NAME}?start=pre_{file.file_id}")),
    ] for file in files]
  else:
    btn = [[
      InlineKeyboardButton(
  ...
  if offset != "":
    ...
    btn.append([
      InlineKeyboardButton(text=f"🗓 1/{math.ceil(int(total_results) / 10)}",
                           callback_data="pages"),
      InlineKeyboardButton(text="Nᴇxᴛ ›››",
                           callback_data=f"next_{req}_{key}_{offset}")
    ])
  else:
    btn.append([InlineKeyboardButton(text="🗓 1/1", callback_data="pages")])
  imdb = await get_poster(
    search, file=(files[0]).file_name) if settings["imdb"] else None
  TEMPLATE = settings['template']
  if imdb:
    cap = TEMPLATE.format(query=search,
                          title=imdb['title'],
                          votes=imdb['votes'],
                          aka=imdb["aka"],
  ...
  if imdb and imdb.get('poster'):
    try:
      hehe = await message.reply_photo(photo=imdb.get('poster'),
                                       caption=cap[:1024],
                                       reply_markup=InlineKeyboardMarkup(btn))
      if AUTO_DELETE:
      ...
      hmm = await message.reply_photo(photo=poster,
                                      caption=cap[:1024],
                                      reply_markup=InlineKeyboardMarkup(btn))
      if AUTO_DELETE:
        await asyncio.sleep(AUTO_DELETE_SECONDS)
      ...
      logger.exception(e)
      fek = await message.reply_text(cap,
                                     reply_markup=InlineKeyboardMarkup(btn))
      if AUTO_DELETE:
        await asyncio.sleep(AUTO_DELETE_SECONDS)
        await fek.delete()
...
async def advantage_spell_chok(msg):
  query = re.sub(
    ...
    "",
    msg.text,
    flags=re.IGNORECASE)  # plis contribute some common words
  ...
  gs_parsed = [
    re.sub(
      ...
      '',
      i,
      flags=re.IGNORECASE) for i in gs
  ]
  if not gs_parsed:
    reg = re.compile(
  ...
  if gs_parsed:
    for mov in gs_parsed:
      imdb_s = await get_poster(mov.strip(),
                                bulk=True)  # searching each keyword in imdb
      if imdb_s:
      ...
      if btn is not None:
        try:
          if fileid == "None":
            ...
            await client.send_cached_media(group_id,
                                           fileid,
