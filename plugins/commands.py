...
from info import CHANNELS, ADMINS, AUTH_CHANNEL, LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, PROTECT_CONTENT, GROUP_LINK
...
two_buttons = [
            [
                InlineKeyboardButton(
                    '🤖 ᴜᴘᴅᴀᴛᴇs 🤖', url=f'{GROUP_LINK}')
            ],
            [
                InlineKeyboardButton(
                    'ℹ️ ʜᴇʟᴘ ℹ️', url=f"https://t.me/{temp.U_NAME}?start=help"),
            ]
        ]
...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        ...
        reply_markup = InlineKeyboardMarkup(two_buttons)
...
print("ADMIN",ADMINS)
@Client.on_message(filters.chat(ADMINS) & filters.command('logs'))
